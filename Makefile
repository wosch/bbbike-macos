...
#
# $Id: Makefile,v 1.110 2009/04/13 14:22:12 wosch Exp $
...
build-perl-intel: 
	...
	cd ${BUILD_DIR}/${PERL_RELEASE};  \
		env PATH=${PATH} HOME=${CPAN_HOME} cc='cc' ccflags='-g -pipe -fno-common -DPERL_DARWIN -no-cpp-precomp -fno-strict-aliasing -Wdeclaration-after-statement -I/usr/include' optimize='-O3' ld='cc -mmacosx-version-min=10.5' ldflags='-L/usr/lib' \
		./Configure -ds -e -Dextras="Tk" -Dinc_version_list=none -Dlocincpth="/usr/include" -Dloclibpth="/usr/lib" -Dprefix=${PERL_FAKEDIR}/${PERL_RELEASE} -Duseithreads -Duseshrplib > perl-config.log 2>&1 
	@echo "build perl..."
	@cd ${BUILD_DIR}/${PERL_RELEASE}; \
		yes "" | ( PATH=${PATH} HOME=${CPAN_HOME} ${MAKE} ${MAKE_ARGS} all && ${MAKE} install ) > make.log 2>&1
